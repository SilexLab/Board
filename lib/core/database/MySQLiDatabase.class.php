...
class MySQLiDatabase extends Database {
	// MySQLi Connection Infos
	private $Host, $Username, $Password, $Database;
	
	//MySQLi Class
	private $DB;
	
	// MySQLi-Query Variables
	...
	public function RowExists($Table, $Where) {
		//$query = $this->DB->query('SELECT * FROM `'.$Table.'` WHERE `Wert` = \'Value\'');
	...
	private function ExecuteQuery() {
		...
		$this->Result = $this->DB->query($this->SQLQuery);
		if(!$this->Result)
			die('<h2>Fatal MySQL Error:</h2>'."\n".
				'	<strong>Errorcode</strong>: '.$this->DB->errno.'<br>'."\n".
				'	<strong>Error</strong>: '.$this->DB->error.'<br>'."\n".
				'	<strong>Query</strong>:
	...
	private function SelectDB() {
		$Select = $this->DB->select_db($this->Database);
		if(!$Select) {
			die('<h2>Fatal Error:</h2>'."\n".
